...
def merge_N_results(result_files, merged_file, intermediate_file):
    merger = BlastpMerger()
    num_results = len(result_files)
    print(num_results)
    file1 = result_files[0]
    for i in range(1, num_results - 1):
        file2 = result_files[i]
        merger.merge_multi(file1, file2, intermediate_file)
        file1 = intermediate_file
        print(file1)

    merger.merge_multi(intermediate_file, result_files[num_results - 1], merged_file)
...
def main(argv):
    ...
    files = argv[2: N + 2]
    print(files)
    merged_file = argv[N + 2]
    intermediate_file = argv[N + 3]
    print(merged_file)
    merge_N_results(files, merged_file, intermediate_file)
